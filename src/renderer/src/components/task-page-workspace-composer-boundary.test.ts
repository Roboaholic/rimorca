...
describe('TaskPage workspace creation source boundaries', () => {
  it('prefills the workspace composer for GitHub issues and pull requests', () => {
    ...
    expect(section).toContain("getTaskPageRepoSourceContext(repo, 'github')")
    ...
    expect(section).toContain('getTaskWorkspaceProjectGroupId(')
    expect(section).toContain('initialProjectGroupId')
